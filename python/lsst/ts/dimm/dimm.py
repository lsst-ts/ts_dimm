...
# Aliases
state = salobj.State
base = salobj.base_csc

SEEING_LOOP_DONE: int = 101
"""
Published in `SALPY_DIMM.DIMM_logevent_errorCodeC` if the coroutine that
gets new seeing data from the controller finishes while the CSC is in enable state.
"""
TELEMETRY_LOOP_DONE: int = 102
"""
Published in `SALPY_DIMM.DIMM_logevent_errorCodeC` if the coroutine that
monitors the health and status of the DIMM finishes while the CSC is in enable state.
"""
class DIMM(salobj.ConfigurableCsc):
    """DIMM CSC

    Commandable SAL Component (CSC) for the DIMM
    """
    def __init__(self, config_dir=None, initial_state=state.STANDBY,
        ...
        super().__init__(name="DIMM", schema_path=schema_path, config_dir=config_dir,
    ...
    async def end_enable(self, id_data):
        ...
        This method will call `start` on the model controller
        and start the telemetry and seeing monitoring loops.
    ...
    async def telemetry_loop(self):
        ...
        This method should only be running if the component is enabled.
        It will get the state of the model controller and output it to
        the telemetry stream at the heartbeat interval.
        """
        ...
        while self.telemetry_loop_running:
            ...
            await asyncio.sleep(base.HEARTBEAT_INTERVAL)

    async def seeing_loop(self):
        """Seeing loop coroutine.

        This method is responsible for getting new measurements from the DIMM
        controller and and output them as events.

        The choice of SAL Events instead of SAL Telemetry comes from the fact
        that the measurements are not periodic.

        They may take different amount of time depending of the star being used to measure seeing,
        be interrupted during the selection of a new target and so on.

        The model controller can just raise an exception in case of an error and the health loop will
    ...
    async def health_monitor(self):
        """
        This loop monitors the health of the DIMM controller and the seeing and telemetry loops.

        If an issue happen it will output the `errorCode` event and put the component in FAULT state.
        """
        while True:
