...
class SimDIMM(BaseDIMM):
    """
    This controller provides a simmulated DIMM interface
    that can be used for testing and mocking a real DIMM.
    """
    ...
    def setup(self, avg_seeing, std_seeing, chance_failure,
              time_in_target, exposure_time):
        """Setup SimDim.
        ...
        if 'min' not in exposure_time or \
           'max' not in exposure_time or \
           'std' not in exposure_time:
    ...
    def new_measurement(self):
        ...
        -------
        measurement : dict
            A dictionary with the same values of
            the dimmMeasurement topic SAL Event.
        """
        self.measurement_start = datetime.datetime.now()

        modified_exptime = \
            self.current_exptime + np.random.uniform(-self.exposure_time['std'] / 2.,
    ...
    def new_hrnum(self):
        """Generate a new target for the DIMM.

        This is basically a new id (hrnum) and exposure time.
        """
